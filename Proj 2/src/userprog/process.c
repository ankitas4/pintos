...
process_execute (const char *file_name) 
{
  ...
  //extract the real file name from str
  char *save_ptr;
  ...
  sema_down(&thread_current()->lock_child);
...
process_wait (tid_t child_tid) 
{
  ...
  thread_current()->wait_for = ch->tid;
    
  if(!ch->used)
    sema_down(&thread_current()->lock_child);

  int temp = ch->ret_status;
  list_remove(e_rm);
...
process_exit (void)
{
  ...
  uint32_t *pd;
    ...
    if(cur->ret_status==-100)
      thread_exit_with(-1);

    int exit_code = cur->ret_status;
    printf("%s: exit(%d)\n",cur->name,exit_code);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  // for rox test, deny write in a elf file
